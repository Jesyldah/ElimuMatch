...
<body>
  ...
  <script>
    ...
    function renderStudents(schoolId) {{
      ...
      const trust = document.getElementById('studentTrust');
      const schoolName = (list[0] && list[0].school)
        || (DATA.schools.find(s => s.id === Number(schoolId)) || {{}}).name
        || 'this school';
      if (trust) {{
        trust.textContent =
          `Real student at ${{schoolName}}. Need verified from school fee records. Reviewed before publish. Gifts pass through to school fee accounts.`;
      }}
